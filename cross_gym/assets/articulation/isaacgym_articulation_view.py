...
class IsaacGymArticulationView:
    ...
    def get_root_orientations(self) -> torch.Tensor:
        """Get root link orientations as quaternions (w, x, y, z).
        
        Returns:
            Tensor of shape (num_envs, 4) in (w, x, y, z) format
        """
        # Isaac Gym uses (x, y, z, w), convert to (w, x, y, z)
        quat_xyzw = self._root_state[:, 3:7]
    ...
    def set_root_state(
        ...
        Args:
            ...
            root_quat: Root orientations as quaternions (num_envs, 4) in (w, x, y, z) format
    ...
    def get_body_orientations(self) -> torch.Tensor:
        """Get all body orientations as quaternions (w, x, y, z).
        
        Returns:
            Tensor of shape (num_envs, num_bodies, 4) in (w, x, y, z) format
        """
        # Isaac Gym uses (x, y, z, w), convert to (w, x, y, z)
        quat_xyzw = self._rigid_body_states[..., 3:7]
        return torch.cat([quat_xyzw[..., 3:4], quat_xyzw[..., :3]], dim=-1)
